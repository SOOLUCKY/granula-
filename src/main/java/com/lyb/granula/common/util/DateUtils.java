...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static int getAge(String birthdayStr) {
        ...
        try{
            if(StringUtils.isNotBlank(birthdayStr)){
                birthday = DateUtil.yyyyMMddToDate(birthdayStr);
            }
        }catch (Exception e){
    ...
    /**
     * 通过生日获取年龄，返回字符串（小于一月的以月计）
     */
    public static String getAgeString(String dateOfBirthStr) {
        Date dateOfBirth = null;
        try{
            dateOfBirth = DateUtil.yyyyMMddToDate(dateOfBirthStr);
        }catch (Exception e){
        ...
        int months=monthNow-monthBirth;
        if(months<0){
            months=12+months;
        }
        if(age<=0) {
